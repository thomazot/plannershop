...
$j(document)
    .ready(function ($) {
        ...
        // Categories Mobile 
        $('.all-li--0.parent').click(function (evt) {
            if ($(evt.target).hasClass('all-li--0')) {
                $(evt.target).toggleClass('on');
            }
        });

        // rulers
        var rulers = $('.banner-rulers__list');
        if (rulers)
            rulers.owlCarousel({
                itemsScaleUp: true,
                navigation: true,
        ...
        if ($('#instafeed').length) {
            var userFeed = new Instafeed({
                get: 'user',
                userId: '2878425202',
                accessToken: '2878425202.1677ed0.599d787c2b73417d97b438ad6facbd44',
